...
**The Emplacement
** is a heavy defensive structure used for protecting the perimeter of your factory. To form it, use a [hammer](introduction#introductionHammer) on the upper middle sandbag block.
# 1
...
# 2
The **Emplacement
** requires [electricity], [ammunition] and by default, a [redstone signal] in order to operate. If the redstone signal is cut off, the weapon will retract back into the emplacement.
...
# 3
**Emplacement
** weapons can be damaged and automatically repaired. You can set up the minimal threshold of the emplacement weapon's health before it goes into repair state.
During repair, it will hide inside the Emplacement and will not react to commands. Note that Emplacements **can be
destroyed
** if enough damage is inflicted to the gun itself, which also means that you will lose resources used to construct it.
# 4
...
# 5
**Attention!
** The Engineering Department(TM) is not liable for any property damage or death inflicted by incorrect targeting inputs.
