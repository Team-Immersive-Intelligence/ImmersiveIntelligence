# meta
Alarm Siren
The reactor is OK... was, at least.
# alarm siren 1
|[crafting]{source:"alarm_siren"}|
The alarm siren is a device which emits a loud noise when provided a redstone signal. Volume depends on signal strength.
To achieve this, [Redstone wire](redstoneWire) can be attached to the siren. Like every redstone connector, it can be reconfigured to use a different redstone channel.