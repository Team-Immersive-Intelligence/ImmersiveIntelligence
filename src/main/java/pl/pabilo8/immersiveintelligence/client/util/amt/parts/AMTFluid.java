...
public class AMTFluid extends AMT
{
	...
	public AMTFluid(String name, AMTModelHeader header, Vec3d size)
	{
		super(name, header);
		this.size = size;
	}
...
//		IIClientUtils.drawFace(buff, 0, 0, 0, -size.x, 0, -size.z, u, ux, v, vv);
	...
	public void applyProperties(EasyNBT nbt)
	{
		...
		nbt.checkSetString("fluid", f -> setFluid(new FluidStack(FluidRegistry.getFluid(f), 1000)));
		nbt.checkSetFloat("level", this::setLevel);
		nbt.checkSetBoolean("flowing", this::setFlowing);
	}
