...
public class ShotgunRenderer extends IIUpgradableItemRendererAMT<ItemIIShotgun> implements ISpecificHandRenderer
{
	...
	{
		super(IIContent.itemShotgun, ResLoc.of(RES_MODEL_WEAPON, "shotgun"));
	}
	...
	public void draw(ItemStack stack, TransformType transform, BufferBuilder buf, Tessellator tes, float partialTicks)
	{
		...
		if(handRender)
		{
			int aiming = nbt.getInt(ItemIIShotgun.AIMING);
		...
		(semiAuto?fireSemiAutomatic: fireStandard).apply(gui?0: (1f-((firing-partialTicks)/(item.getFireDelay(stack, nbt)))));
	...
	{
		this.handmadeRemapper = new MTLTextureRemapper(model, ResLoc.of(directoryRes, "shotgun_handmade").withExtension(ResLoc.EXT_MTL));
		...
		this.model = AMTModelCacheBuilder.startItemModel()
				...
				.withTextureProvider(
						(res, stack) ->
						{
							String skin = IIContent.itemShotgun.getSkinnableCurrentSkin(stack);
							if(IISkinHandler.isValidSkin(skin))
							{
								this.skinRemapper = new MTLTextureRemapper(model, ResLoc.of(IIReference.RES_TEXTURES_SKIN, skin, "/shotgun").withExtension(ResLoc.EXT_MTL));
								return ClientUtils.getSprite(this.skinRemapper.apply(res));
							}

							if(ItemNBTHelper.hasKey(stack, ItemIIShotgun.HANDMADE))
