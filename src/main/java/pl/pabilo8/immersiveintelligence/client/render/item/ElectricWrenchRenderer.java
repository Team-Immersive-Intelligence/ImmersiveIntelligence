 ...
 * @since 13.10.2019
 */
@RegisteredItemRenderer(name = "items/tools/electric_wrench")
public class ElectricWrenchRenderer extends IIItemRendererAMT<ItemIIElectricWrench> implements ISpecificHandRenderer
{
	private AMTModel model;
	private IIAnimationCompiledMap wrenching, equip, hand;

	public ElectricWrenchRenderer()
	{
		super(IIContent.itemElectricWrench, ResLoc.of(IIReference.RES_ITEM_MODEL, "tools/electric_wrench")
	...
	public void draw(ItemStack stack, TransformType transform, BufferBuilder buf, Tessellator tes, float partialTicks)
	{
		...
		if(is1stPerson(transform))
		{
			...
			if(itemEquipTime==1&&item.hasEnoughEnergy(stack))
				wrenching.apply(AMTUtils.getDebugProgress(24, partialTicks));
		}
		else
			hand.apply(0);
	...
	public void compileModels(OBJModel model, AMTModelHeader header)
	{
		...
		this.wrenching = IIAnimationCompiledMap.create(this.model, ResLoc.of(IIReference.RES_II, "tools/electric_wrench/wrench"));
		this.equip = IIAnimationCompiledMap.create(this.model, ResLoc.of(IIReference.RES_II, "tools/electric_wrench/equip"));
		this.hand = IIAnimationCompiledMap.create(this.model, ResLoc.of(IIReference.RES_II, "tools/electric_wrench/hand"));
	}
