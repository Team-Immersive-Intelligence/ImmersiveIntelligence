...
public class AdvancedInserterRenderer extends TileEntitySpecialRenderer<TileEntityAdvancedInserter> implements IReloadableModelContainer<AdvancedInserterRenderer>
{
	public static ItemStack conn_data, conn_mv;
	static RenderItem renderItem = ClientUtils.mc().getRenderItem();
	private static ModelAdvancedInserter model;
	private static final ResourceLocation TEXTURE = new ResourceLocation(ImmersiveIntelligence.MODID+":textures/blocks/metal_device/advanced_inserter.png");

	@Override
	public void render(@Nullable TileEntityAdvancedInserter te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
	{
		if(te!=null)
		{
	...
	{
		model = new ModelAdvancedInserter();
