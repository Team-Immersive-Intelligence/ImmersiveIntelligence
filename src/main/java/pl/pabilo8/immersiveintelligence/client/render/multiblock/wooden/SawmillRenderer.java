 ...
 */
public class SawmillRenderer extends TileEntitySpecialRenderer<TileEntitySawmill>
{
	static RenderItem renderItem = ClientUtils.mc().getRenderItem();
	private static ModelSawmill model = new ModelSawmill();
	...
	public void render(TileEntitySawmill te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
	{
		if(te!=null&&!te.isDummy())
		{
			...
			if(te.processPrimary.getCount() > 0)
			{
				...
				else if(timeFull < 0.875f)
				{
					...
					if(time_half||iteration > 0)
					{
						...
						GlStateManager.translate(2.5, 1.25, -0.325f);
				...
				GlStateManager.translate(0, 0, 0.5+(Math.min((iteration+inserter_backpush_time)/(float)te.processPrimary.getCount(), 1)*-0.325)-inserter_push);
				for(ModelRendererTurbo mod : model.inserterMovingPartModel)
					mod.render(0.0625f);

				GlStateManager.translate(3.5, 1.25, -0.325f);
				...
				if(!te.inventory.get(2).isEmpty()||iteration > 0)
				{
					...
					GlStateManager.translate(0.25, 0.15, -0.25);
					...
					for(int i = 0; i < total; i += 1)
					{
						GlStateManager.pushMatrix();
						GlStateManager.rotate(45f, 1, 0, 0);
						GlStateManager.translate(i%3*0.25f, (i/3)*0.25f, (i/3)*-0.15);
						GlStateManager.rotate(-45f, 1, 0, 0);
			...
			{
				GlStateManager.pushMatrix();
				GlStateManager.translate(3.5, 1.25, -0.35f);
