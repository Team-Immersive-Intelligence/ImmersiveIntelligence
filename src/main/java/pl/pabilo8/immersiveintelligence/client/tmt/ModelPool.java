...
public class ModelPool
{
	public static ModelPoolEntry addFile(String file, Class modelClass, Map<String, TransformGroup> group, Map<String, TextureGroup> textureGroup)
	{
		...
		{
			ImmersiveIntelligence.logger.error("A new "+entry.getClass().getName()+" could not be initialized.");
			ImmersiveIntelligence.logger.error(e.getMessage());
			return null;
		}
		File modelFile = null;
		...
		{
			ImmersiveIntelligence.logger.warn("The model with the name "+file+" does not exist.");
			return null;
		}
