...
import pl.pabilo8.immersiveintelligence.common.blocks.multiblocks.metal.tileentities.first.TileEntityPacker.PackerActionType;
import pl.pabilo8.immersiveintelligence.common.blocks.multiblocks.metal.tileentities.first.TileEntityPacker.PackerPutMode;
...
public class GuiPacker extends GuiIEContainerBase
{
	...
	public void initGui()
	{
		...
		addLabel(5, 6, 78, 11, Utils.COLOR_H1, "Tasks");
		...
		if(taskList.selectedOption!=-1)
		{
			...
			if(task!=null)
			{
				addLabel(84, 10, 89, 0, Utils.COLOR_H1, "Task: "+
						I18n.format(CommonProxy.DESCRIPTION_KEY+"metal_multiblock1.packer.task."+task.actionType.getActionName(tile.hasUpgrade(IIContent.UPGRADE_UNPACKER_CONVERSION)))
				).setCentered();

				addLabel(88, 26, Utils.COLOR_H1, "Item:");

				addLabel(88, 42, Utils.COLOR_H1, "Mode:");
				putModeList = addButton(new GuiButtonDropdownList(buttonList.size(), guiLeft+84+32, guiTop+10+6+20, 52, 20,
						...
						Arrays.stream(PackerPutMode.values())
								...
								.toArray(String[]::new)));
				putModeList.setTranslationFunc(s -> I18n.format(CommonProxy.DESCRIPTION_KEY+"metal_multiblock1.packer.mode."+s));
				putModeList.selectedEntry = task.mode.ordinal();

				addLabel(88, 54, Utils.COLOR_H1, "Amount:");
				switchOreDict = switchOreDict==null?addSwitch(88, 78, 85, Utils.COLOR_H2, 0xb51500, 0x95ed00, task.stack.oreName!=null, "Uses OreDict", false): addButton(switchOreDict);
				switchNBT = switchNBT==null?addSwitch(88, 90, 85, Utils.COLOR_H2, 0xb51500, 0x95ed00, task.stack.useNBT, "NBT Sensitive:", false): addButton(switchNBT);
