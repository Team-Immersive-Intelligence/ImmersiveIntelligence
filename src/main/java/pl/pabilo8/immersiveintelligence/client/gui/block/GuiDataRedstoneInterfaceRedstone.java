...
public class GuiDataRedstoneInterfaceRedstone extends GuiIEContainerBase implements ITabbedGui
{
	...
	public void initGui()
	{
		...
		this.buttonList.add(new GuiButtonIE(0, guiLeft-28, guiTop+4, 28, 24, "", texture, 176, 0).setHoverOffset(28, 0));
		this.buttonList.add(new GuiButtonIE(1, guiLeft-28, guiTop+28, 28, 24, "", texture, 204, 24));
	}
	...
	protected void actionPerformed(GuiButton button)
	{
		...
		{
			syncDataToServer();
			IIPacketHandler.sendToServer(new MessageGuiNBT(IIGUI.DATA_REDSTONE_INTERFACE_DATA, tile));
	...
	{
		this.fontRenderer.drawString(I18n.format("tile."+ImmersiveIntelligence.MODID+".metal_multiblock.redstone_interface.redstone"), 4, 2, DecoTextures.COLOR_H1.getPackedRGB());
	}
	...
	public void drawScreen(int mx, int my, float partial)
	{
		...
		for(char c : DataPacket.VARIABLE_NAMES)
			if(list.has(c))
			{
				...
				this.fontRenderer.drawString(String.valueOf(c), drawx+38, drawy+5, Lib.COLOUR_I_ImmersiveOrange, true);
	...
	{
		syncDataToServer();
		IIPacketHandler.sendToServer(new MessageBooleanAnimatedPartsSync(0, false, tile));
		IIPacketHandler.sendToServer(new MessageBooleanAnimatedPartsSync(1, false, tile));
		super.onGuiClosed();
	}
