...
public class GuiChemicalPainter extends GuiIEContainerBase implements ISlider
{
	...
	public void initGui()
	{
		...
		switch(colorMode)
		{
			case RGB:
			{
				float[] rgb = Utils.rgbIntToRGB(color);
	...
	{
		Utils.drawStringCentered(this.fontRenderer, I18n.format("tile.immersiveintelligence.metal_multiblock1.chemical_painter.name"),
				8, 6, xSize-12, 0, Utils.COLOR_H1);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int mx, int my)
	{
		...
		Utils.drawPowerBar(guiLeft+159, guiTop+23, 7, 47, tile.getEnergyStored(null)/(float)tile.getMaxEnergyStored(null));
	...
	public void drawScreen(int mx, int my, float partial)
	{
		...
		if(mx > guiLeft+159&&mx < guiLeft+166&&my > guiTop+23&&my < guiTop+70)
			...
			tooltip.add(I18n.format(CommonProxy.DESCRIPTION_KEY+"metal_multiblock1.chemical_painter.color.rgb"));
		else if(buttonModeCMYK.isMouseOver())
			tooltip.add(I18n.format(CommonProxy.DESCRIPTION_KEY+"metal_multiblock1.chemical_painter.color.cmyk"));
		else if(buttonModeHSV.isMouseOver())
			tooltip.add(I18n.format(CommonProxy.DESCRIPTION_KEY+"metal_multiblock1.chemical_painter.color.hsv"));
		else if(buttonActiveColor.isMouseOver())
			tooltip.add(
					colorDelay > 0?
							TextFormatting.GREEN+TextFormatting.ITALIC.toString()+I18n.format(CommonProxy.DESCRIPTION_KEY+"metal_multiblock1.chemical_painter.color_copied")+TextFormatting.RESET:
							String.format("<hexcol=%1$s:#%1$s (%2$s)>",
									String.format("%06x", color),
									I18n.format("item.fireworksCharge."+Utils.getRGBTextFormatting(color).getName()))
			);
		else
		{
			for(int i = 0; i < 4; i++)
	...
	{
		GuiSliderII slider = addButton(new GuiSliderII(buttonList.size(), guiLeft+25+((id%2)*75), guiTop+80+8+(17*(int)Math.floor(id/2f)), 70, name, value, Utils.COLOR_H1));
		slider.parent = this;
		return slider;
	}
	...
	public void onChangeSliderValue(GuiSlider slider)
	{
		...
		{
			case RGB:
				color = Utils.rgb((float)sliderRed.sliderValue, (float)sliderGreen.sliderValue, (float)sliderBlue.sliderValue);
				break;
			case CMYK:
			{
				float[] rgb = Utils.cmykToRgb((float)sliderCyan.sliderValue, (float)sliderMagenta.sliderValue, (float)sliderYellow.sliderValue, (float)sliderBlack.sliderValue);
				color = Utils.rgb(rgb[0], rgb[1], rgb[2]);
			}
			break;
			case HSV:
			{
				float[] rgb = Utils.hsvToRgb((float)sliderHue.sliderValue, (float)sliderSaturation.sliderValue, (float)sliderValue.sliderValue);
				color = Utils.rgb(rgb[0], rgb[1], rgb[2]);
			}
			break;
		}
