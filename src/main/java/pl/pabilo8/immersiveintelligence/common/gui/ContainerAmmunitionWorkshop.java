...
{
	public ContainerAmmunitionWorkshop(InventoryPlayer inventoryPlayer, TileEntityAmmunitionWorkshop tile)
	{
		...
		for(int i = 0; i < 3; i++)
			for(int j = 0; j < 9; j++)
				addSlotToContainer(new Slot(inventoryPlayer, j+i*9+9, 30+j*18, 95+i*18));
		for(int i = 0; i < 9; i++)
			addSlotToContainer(new Slot(inventoryPlayer, i, 30+i*18, 165-27+15));
	}
}
