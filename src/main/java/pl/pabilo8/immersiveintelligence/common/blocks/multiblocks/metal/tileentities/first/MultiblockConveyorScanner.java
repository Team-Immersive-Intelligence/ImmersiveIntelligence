...
public class MultiblockConveyorScanner implements IMultiblock
{
	static IngredientStack[] materials = new IngredientStack[]{
			new IngredientStack(new ItemStack(IIContent.blockMetalDecoration, 1, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta())),
	...
	static
	{
		structure[0][0][0] = new ItemStack(IIContent.blockMetalDecoration, 1, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta());
		...
		for(int h = -1; h < 2; h++)
			for(int l = 0; l < 1; l++)
				for(int w = 0; w < 1; w++)
				{
					...
					world.setBlockState(pos2, IIContent.blockMetalMultiblock0.getStateFromMeta(IIBlockTypes_MetalMultiblock0.CONVEYOR_SCANNER.getMeta()));
