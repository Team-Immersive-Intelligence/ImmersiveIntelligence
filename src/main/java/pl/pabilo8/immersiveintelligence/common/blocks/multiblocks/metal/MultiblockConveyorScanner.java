...
public class MultiblockConveyorScanner implements IMultiblock
{
	static final IngredientStack[] materials = new IngredientStack[]{
			new IngredientStack(new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta())),
			new IngredientStack(ConveyorHandler.getConveyorStack(ImmersiveEngineering.MODID+":covered")),
	...
	static ItemStack[][][] structure = new ItemStack[3][1][1];

	static
	{
		structure[0][0][0] = new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta());
		...
		for(int h = -1; h < 2; h++)
			for(int l = 0; l < 1; l++)
				for(int w = 0; w < 1; w++)
				{
					...
					world.setBlockState(pos2, CommonProxy.block_metal_multiblock.getStateFromMeta(IIBlockTypes_MetalMultiblock.CONVEYOR_SCANNER.getMeta()));
