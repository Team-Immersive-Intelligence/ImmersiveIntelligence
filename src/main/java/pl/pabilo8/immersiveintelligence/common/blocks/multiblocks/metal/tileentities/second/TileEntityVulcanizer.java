...
public class TileEntityVulcanizer extends TileEntityMultiblockMetal<TileEntityVulcanizer, VulcanizerRecipe> implements IPlayerInteraction, ISoundTile, IGuiTile
{
	...
	public void update()
	{
		...
		if(world.isRemote)
		{
			...
			if(t3!=null)
				ImmersiveEngineering.proxy.handleTileSound(IISounds.inserter_backward, t3, shoudlPlaySound("immersiveintelligence:inserter_backward"), 1.5f, 0.5f);
			if(t4!=null)
				ImmersiveEngineering.proxy.handleTileSound(IISounds.inserter_forward, t4, shoudlPlaySound("immersiveintelligence:inserter_forward"), 1.5f, 0.5f);
		...
		for(MultiblockProcess<VulcanizerRecipe> process : processQueue)
		{
			...
			if(process.processTick==Math.ceil(0.86*process.maxTicks))
				...
				world.playSound(null, getBlockPosForPos(49), IISounds.vulcanizer_pull_end, SoundCategory.BLOCKS, 1, 1f);
			else if(process.processTick==Math.ceil(0.83*process.maxTicks))
				world.playSound(null, getBlockPosForPos(49), IISounds.vulcanizer_pull_start, SoundCategory.BLOCKS, 1, 1f);
			else if(process.processTick==Math.ceil(0.835*process.maxTicks))
				world.playSound(null, getBlockPosForPos(73), IISounds.vulcanizer_pull_start, SoundCategory.BLOCKS, 1, 0.5f);
			else if(process.processTick==Math.ceil(0.85*process.maxTicks))
				world.playSound(null, getBlockPosForPos(73), IISounds.vulcanizer_pull_end, SoundCategory.BLOCKS, 1, 0.5f);
			else if(process.processTick==Math.ceil(0.91*process.maxTicks))
				world.playSound(null, getBlockPosForPos(49), IISounds.vulcanizer_pull_end, SoundCategory.BLOCKS, 1, 1f);
			else if(process.processTick==Math.ceil(0.93*process.maxTicks))
				world.playSound(null, getBlockPosForPos(73), IISounds.vulcanizer_pull_start, SoundCategory.BLOCKS, 1, 1f);
