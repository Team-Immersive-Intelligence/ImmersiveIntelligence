...
import static pl.pabilo8.immersiveintelligence.Config.IIConfig.Machines.conveyor_scanner;
...
public class TileEntityConveyorScanner extends TileEntityMultiblockMetal<TileEntityConveyorScanner, IMultiblockRecipe> implements IDataDevice, IAdvancedCollisionBounds, IAdvancedSelectionBounds, IConveyorAttachable
{
	...
	{
		super(MultiblockConveyorScanner.instance, new int[]{3, 1, 1}, conveyor_scanner.energyCapacity, false);
	}
	...
	public void update()
	{
		...
		if(pos==1&&energyStorage.getEnergyStored() > 0&&!inventory.get(0).isEmpty())
		{
			...
			if(processTime >= 20)
			{
				...
				energyStorage.extractEnergy(conveyor_scanner.energyUsage, false);
