...
public class TileEntityChemicalPainter extends TileEntityMultiblockMetal<TileEntityChemicalPainter, PaintingRecipe> implements IGuiTile, ISoundTile, IAdvancedCollisionBounds, IAdvancedSelectionBounds, ILightProvider, IDataDevice
{
	...
	public void update()
	{
		...
		if(world.isRemote)
		{
			...
			ImmersiveEngineering.proxy.handleTileSound(IISounds.chemical_painter_lights, this, shoudlPlaySound("immersiveintelligence:chemical_painter_lights"), 1.5f, 1f);
			if(t1!=null)
				ImmersiveEngineering.proxy.handleTileSound(IESounds.spray, t1, shoudlPlaySound("immersiveengineering:spray"), 1.5f, 0.5f);
			if(t2!=null)
				ImmersiveEngineering.proxy.handleTileSound(IISounds.chemical_painter_lift_up, t2, shoudlPlaySound("immersiveintelligence:chemical_painter_lift_up"), 1.5f, 0.75f);
			if(t3!=null)
				ImmersiveEngineering.proxy.handleTileSound(IISounds.chemical_painter_lift_down, t2, shoudlPlaySound("immersiveintelligence:chemical_painter_lift_down"), 1.5f, 0.75f);
		...
		if(energyStorage.getEnergyStored() > 0&&processQueue.size() < this.getProcessQueueMaxLength())
		{
			if(Arrays.stream(tanks).anyMatch(ft -> ft.getFluidAmount() > 0))
			{
				...
				if(recipe!=null)
				{
					float[] cmyk = pl.pabilo8.immersiveintelligence.api.Utils.rgbToCmyk(pl.pabilo8.immersiveintelligence.api.Utils.rgbIntToRGB(color));
		...
		pl.pabilo8.immersiveintelligence.api.Utils.handleBucketTankInteraction(tanks, inventory, 2, 3, 0, false, CYAN);
		pl.pabilo8.immersiveintelligence.api.Utils.handleBucketTankInteraction(tanks, inventory, 2, 3, 1, false, MAGENTA);
		pl.pabilo8.immersiveintelligence.api.Utils.handleBucketTankInteraction(tanks, inventory, 2, 3, 2, false, YELLOW);
		pl.pabilo8.immersiveintelligence.api.Utils.handleBucketTankInteraction(tanks, inventory, 2, 3, 3, false, BLACK);
	...
	private void spawnPaintParticle()
	{
		...
		float[] rgb = pl.pabilo8.immersiveintelligence.api.Utils.rgbIntToRGB(color);
		...
		if(master!=null&&master.processQueue.size() > 0)
		{
			switch(sound)
			{
				case "immersiveintelligence:chemical_painter_lights":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(master.processTime, master.processTimeMax, 0.65f, 0.95f);
				case "immersiveengineering:spray":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(master.processTime, master.processTimeMax, 0.25, 0.75);
				case "immersiveintelligence:chemical_painter_lift_up":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(master.processTime, master.processTimeMax, 0.05, 0.2);
				case "immersiveintelligence:chemical_painter_lift_down":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(master.processTime, master.processTimeMax, 0.7, 0.85);
			}
		}
	...
	public Light provideLight()
	{
		if(pos==47)
		{
			...
			{
				if(pl.pabilo8.immersiveintelligence.api.Utils.inRange(master.processTime, master.processTimeMax, 0.65f, 0.95f))
