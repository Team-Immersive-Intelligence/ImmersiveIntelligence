...
public class TileEntityMechanicalPump extends TileEntityIEBase implements ITickable, IBlockBounds, IHasDummyBlocks, IConfigurableSides, IFluidPipe, IBlockOverlayText, IRotationalEnergyBlock, IDirectionalTile
{
	...
	private boolean handleRotation()
	{
		...
		if(te!=null&&te.hasCapability(CapabilityRotaryEnergy.ROTARY_ENERGY, facing))
		{
			...
			{
				IIPacketHandler.INSTANCE.sendToAllAround(new MessageRotaryPowerSync(rotation, 0, getPos()), pl.pabilo8.immersiveintelligence.api.Utils.targetPointFromTile(this, 24));
			}
		}
		else
			b = true;
	...
	{
		for(int i = 0; i <= 1; i++)
			if(Utils.isBlockAt(world, getPos().add(0, dummy?-1: 0, 0).add(0, i, 0), CommonProxy.block_mechanical_device1, IIBlockTypes_MechanicalDevice1.MECHANICAL_PUMP.getMeta()))
