...
public class TileEntityProjectileWorkshop extends TileEntityMultiblockMetal<TileEntityProjectileWorkshop, IMultiblockRecipe> implements ISoundTile, IGuiTile, IAdvancedSelectionBounds, IAdvancedCollisionBounds, IUpgradableMachine, IBooleanAnimatedPartsBlock, IConveyorAttachable, IDataDevice
{
	...
	//for core production
	@Nonnull
	public IBullet producedBullet = IIContent.itemAmmoArtillery;
	@Nonnull
	...
	public void update()
	{
				...
				if(componentInside.amount < ProjectileWorkshop.componentCapacity)
				{
					if(componentInside.isEmpty())
					{
						if(tanksFiller[0].getFluidAmount() >= 1000)
						{
							...
							BulletRegistry.INSTANCE.registeredComponents.values().stream()
					...
					if(productionProgress <= 0)
					{
						if(!inventory.get(0).isEmpty()&&!componentInside.isEmpty())
						{
							...
							IBullet bullet = (IBullet)effect.getItem();
							if(componentInside.matches(bullet))
							{
								...
								{
									bullet.addComponents(effect, BulletRegistry.INSTANCE.getComponent(componentInside.name), componentInside.tagCompound);
		...
		else //production
		{
				...
				if(productionProgress <= 0)
				{
					...
					{
						Optional<IBulletCore> first = BulletRegistry.INSTANCE.registeredBulletCores.values()
								.stream()
		...
		if(update)
		{
			...
			ImmersiveEngineering.packetHandler.sendToAllAround(new MessageTileSync(this, nbt), pl.pabilo8.immersiveintelligence.api.Utils.targetPointFromTile(this,32));
	...
	public void readCustomNBT(@Nonnull NBTTagCompound nbt, boolean descPacket)
	{
		...
		{
			IBullet bb = BulletRegistry.INSTANCE.getBulletItem(nbt.getString("produced_bullet"));
	...
	public void receiveMessageFromServer(@Nonnull NBTTagCompound message)
	{
		...
		{
			IBullet bb = BulletRegistry.INSTANCE.getBulletItem(message.getString("produced_bullet"));
			producedBullet = bb==null?IIContent.itemAmmoArtillery: bb;
		}
	...
	public boolean isStackValid(int i, ItemStack stack)
	{
		...
		{
			return BulletRegistry.INSTANCE.registeredComponents.values().stream().anyMatch(comp -> comp.getMaterial().matchesItemStackIgnoringSize(stack));
		}
		else if(i==0)
		{
			if(fillerUpgrade)
				return stack.getItem() instanceof IBullet&&((IBullet)stack.getItem()).isBulletCore(stack);
			else
				return BulletRegistry.INSTANCE.registeredBulletCores.values().stream().anyMatch(core -> core.getMaterial().matchesItemStackIgnoringSize(stack));
		}
		return false;
	}
		...
		if(master!=null&&master.processQueue.size() > 0)
		{
			...
			switch(sound)
			{
				case "immersiveintelligence:printing_press":
				{
					...
					{
						if(pl.pabilo8.immersiveintelligence.api.Utils.inRange(master.processQueue.get(1).processTick, master.processQueue.get(1).maxTicks, 0, 0.16))
							return true;
					}
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(process.processTick, process.maxTicks, 0, 0.165);
				}
				case "immersiveintelligence:vulcanizer_heating":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(process.processTick, process.maxTicks, 0.2, 0.8);
				case "immersiveintelligence:howitzer_rotation_h":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(process.processTick, process.maxTicks, 0.78, 0.84);
				case "immersiveintelligence:inserter_forward":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(process.processTick, process.maxTicks, 0.93, 0.96);
				case "immersiveintelligence:inserter_backward":
					return pl.pabilo8.immersiveintelligence.api.Utils.inRange(process.processTick, process.maxTicks, 0.85, 0.86);
			}
		}
		 */
	...
	public void onAnimationChangeServer(boolean state, int part)
	{
		...
		if(state!=isDrawerOpened[part])
			world.playSound(null, getPos(), state?IISounds.metal_breadbox_open: IISounds.metal_breadbox_close, SoundCategory.BLOCKS, 0.5F, 0.5f);

		this.isDrawerOpened[part] = state;

		for(int i = 0; i < 2; i++)
			IIPacketHandler.INSTANCE.sendToAllAround(new MessageBooleanAnimatedPartsSync(this.isDrawerOpened[i], i, getPos()), pl.pabilo8.immersiveintelligence.api.Utils.targetPointFromPos(this.getPos(), this.world, 32));
	}
	...
	public void onReceive(DataPacket packet, @Nullable EnumFacing side)
	{
		if(pos==8)
		{
			...
			if(packet.hasVariable('b'))
				master.producedBullet = BulletRegistry.INSTANCE.registeredBulletItems.getOrDefault(packet.getPacketVariable('b').valueToString(), IIContent.itemAmmoArtillery);
	...
	{
		@Nullable
		private IBulletComponent component;
		private String name;
		private int amount;
		@Nonnull
		...
		private BulletComponentStack(String name, int amount, @Nonnull NBTTagCompound tag)
		{
			...
			Optional<IBulletComponent> first = BulletRegistry.INSTANCE.registeredComponents.values().stream()
		...
		private BulletComponentStack(IBulletComponent component, @Nullable NBTTagCompound tag)
		{
		...
		public boolean matches(IBullet bullet)
		{
		...
		public IBulletComponent getComponent()
		{
			return component;
		}
	}
