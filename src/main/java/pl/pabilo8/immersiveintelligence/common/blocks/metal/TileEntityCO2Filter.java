...
public class TileEntityCO2Filter extends TileEntityIEBase implements ITickable, IBlockBounds, IDirectionalTile, IHasDummyBlocks
{
	...
	public void placeDummies(BlockPos pos, IBlockState state, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		for(int i = 1; i <= 2; i++)
		{
			...
			((TileEntityCO2Filter)world.getTileEntity(pos.add(0, i, 0))).dummy = i;
	...
	{
		for(int i = 0; i <= 2; i++)
			if(world.getTileEntity(getPos().add(0, -dummy, 0).add(0, i, 0)) instanceof TileEntityCO2Filter)
	...
	{
		if(dummy==1&&facing==(this.getFacing()==EnumFacing.UP?EnumFacing.NORTH: this.facing)&&capability==CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY)
			return true;
