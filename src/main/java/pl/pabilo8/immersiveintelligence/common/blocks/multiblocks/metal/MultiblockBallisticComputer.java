...
public class MultiblockBallisticComputer implements IMultiblock
{
	static final IngredientStack[] materials = new IngredientStack[]{
			...
			new IngredientStack(new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 3, IIBlockTypes_MetalDecoration.ELECTRONIC_ENGINEERING.getMeta())),
			new IngredientStack(new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.COIL_DATA.getMeta())),
		...
		for(int h = 0; h < 2; h++)
		{
			for(int l = 0; l < 2; l++)
			{
				for(int w = 0; w < 2; w++)
				{
					...
					if(h==0&&w==0&&l==0)
					{
						structure[h][l][w] = new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.COIL_DATA.getMeta());
					}
					else if(l==0)
					{
	...
	{
		return state.getBlock()==ImmersiveIntelligence.proxy.block_metal_decoration&&
		...
		for(int h = 0; h < 2; h++)
			for(int l = 0; l < 2; l++)
				for(int w = 0; w < 2; w++)
				{
					...
					world.setBlockState(pos2, CommonProxy.block_metal_multiblock.getStateFromMeta(IIBlockTypes_MetalMultiblock.BALLISTIC_COMPUTER.getMeta()));
		...
		for(int h = 0; h < 2; h++)
			for(int l = 0; l < 2; l++)
				for(int w = 0; w < 2; w++)
				{
					...
					if(h==0&&w==0&&l==0)
					{
						if(!Utils.isBlockAt(world, pos, ImmersiveIntelligence.proxy.block_metal_decoration, IIBlockTypes_MetalDecoration.COIL_DATA.getMeta()))
						{
							return false;
						}
					}
