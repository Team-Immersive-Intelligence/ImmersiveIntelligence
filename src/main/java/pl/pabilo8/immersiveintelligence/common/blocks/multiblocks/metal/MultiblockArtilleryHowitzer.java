...
public class MultiblockArtilleryHowitzer implements IMultiblock
{
	...
	static final IngredientStack[] materials = new IngredientStack[]{
			...
			new IngredientStack(new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 12, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta())),
			new IngredientStack(new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.ELECTRONIC_ENGINEERING.getMeta())),
			...
			for(int l = 0; l < 9; l++)
			{
				for(int w = 0; w < 9; w++)
				{
					...
					else if(((w >= 0&&w <= 2&&l >= 0&&l <= 2)||(w >= 0&&w <= 2&&l >= 6&&l <= 8)||(w >= 6&&w <= 8&&l >= 0&&l <= 2)||(w >= 6&&w <= 8&&l >= 6&&l <= 8))&&h < 5)
					{
						...
						structure[h][l][w] = new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta());
					...
					else if(h==5)
					{
						...
						else if(l >= 3&&l <= 5&&(w==0||w==8))
						{
							if(l==4&&w==0)
								...
								structure[h][l][w] = new ItemStack(ImmersiveIntelligence.proxy.block_metal_decoration, 1, IIBlockTypes_MetalDecoration.ELECTRONIC_ENGINEERING.getMeta());
							else
								structure[h][l][w] = new ItemStack(IEContent.blockStorage, 1, BlockTypes_MetalsIE.STEEL.getMeta());
						}
						else if(w==4&&l==0)
			...
			for(int l = 0; l < 9; l++)
				for(int w = -4; w < 5; w++)
				{
					...
					for(BlockPos posv : checklist)
					{
						...
						{
							ImmersiveIntelligence.logger.info(pos2);
							valid = false;
							break;
						}
					}
					if(!valid)
					{
						ImmersiveIntelligence.logger.info("air block! at "+pos2);
						continue;
					}

					world.setBlockState(pos2, CommonProxy.block_metal_multiblock.getStateFromMeta(IIBlockTypes_MetalMultiblock.ARTILLERY_HOWITZER.getMeta()));
			...
			for(int l = 0; l < 9; l++)
				for(int w = -4; w < 5; w++)
				{
					...
					if(h==-5)
					{
						if((l==2&&w==-2)||(l==2&&w==2)||(l==6&&w==-2)||(l==6&&w==2))
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockStoneDecoration, BlockTypes_StoneDecoration.CONCRETE.getMeta()))
							{
								...
								ImmersiveIntelligence.logger.error("error at: "+h+" "+w+" "+l);
								return false;
					...
					else if(((w >= -4&&w <= -2&&l >= 0&&l <= 2)||(w >= -4&&w <= -2&&l >= 6&&l <= 8)||(w >= 2&&w <= 4&&l >= 0&&l <= 2)||(w >= 2&&w <= 4&&l >= 6&&l <= 8))&&h < 0)
					{
						...
						{
							ImmersiveIntelligence.logger.error("1 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					else if(w < -2&&l <= 5&&l >= 3&&h < 0)
					{
						if(!Utils.isBlockAt(world, pos, IEContent.blockMetalDecoration0, BlockTypes_MetalDecoration0.HEAVY_ENGINEERING.getMeta()))
						{
							ImmersiveIntelligence.logger.error("2 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					else if(w >= 3&&l <= 8&&l >= 3&&h < 0)
					{
						if(!Utils.isBlockAt(world, pos, ImmersiveIntelligence.proxy.block_metal_decoration, IIBlockTypes_MetalDecoration.ADVANCED_ELECTRONIC_ENGINEERING.getMeta()))
						{
							ImmersiveIntelligence.logger.error("3 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					else if(w >= -1&&w <= 1&&l <= 1&&h < 0)
					{
						if(!Utils.isBlockAt(world, pos, IEContent.blockMetalDecoration0, BlockTypes_MetalDecoration0.HEAVY_ENGINEERING.getMeta()))
						{
							ImmersiveIntelligence.logger.error("4 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					else if(w >= -1&&w <= 1&&l >= 7&&l <= 8&&h < 0)
					{
						if(!Utils.isBlockAt(world, pos, IEContent.blockMetalDecoration0, BlockTypes_MetalDecoration0.HEAVY_ENGINEERING.getMeta()))
						{
							ImmersiveIntelligence.logger.error("5 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					...
					else if(l==4&&w==0&&h >= -4&&h <= 0)
					{
						...
						{
							ImmersiveIntelligence.logger.error("6 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					//Cannon
					else if(h==-2&&w >= -1&&w <= 1&&l >= 3&&l <= 5&&w!=0)
					{
						...
						{
							ImmersiveIntelligence.logger.error("7 error at: "+h+" "+w+" "+l);
							return false;
						}
					}
					...
					else if(h==0)
					{
						if(w >= -1&&w <= 1&&(l==1||l==7))
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockStoneDecoration, BlockTypes_StoneDecoration.CONCRETE.getMeta()))
							{
								ImmersiveIntelligence.logger.error("8 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						else if(l >= 3&&l <= 5&&(w==-3||w==3))
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockStoneDecoration, BlockTypes_StoneDecoration.CONCRETE.getMeta()))
							{
								ImmersiveIntelligence.logger.error("9 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						...
						else if(w >= -3&&w <= -2&&l >= 1&&l <= 2&&!(w==-3&&l==1))
						{
							...
							{
								ImmersiveIntelligence.logger.error("10 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						else if(w >= 2&&w <= 3&&l >= 1&&l <= 2&&!(w==3&&l==1))
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockWoodenDecoration, BlockTypes_WoodenDecoration.SCAFFOLDING.getMeta()))
							{
								ImmersiveIntelligence.logger.error("11 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						else if(w >= -3&&w <= -2&&l >= 6&&l <= 7&&!(w==-3&&l==7))
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockWoodenDecoration, BlockTypes_WoodenDecoration.SCAFFOLDING.getMeta()))
							{
								ImmersiveIntelligence.logger.error("12 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						else if(w >= 2&&w <= 3&&l >= 6&&l <= 7&&!(w==3&&l==7))
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockWoodenDecoration, BlockTypes_WoodenDecoration.SCAFFOLDING.getMeta()))
							{
								ImmersiveIntelligence.logger.error("13 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						...
						else if(w >= -1&&w <= 1&&l==8)
						{
							if(w!=0)
							{
								if(!Utils.isBlockAt(world, pos, IEContent.blockStorage, BlockTypes_MetalsIE.STEEL.getMeta()))
								{
									ImmersiveIntelligence.logger.error("14 error at: "+h+" "+w+" "+l);
									return false;
								}
							}
							else
							{
								...
								{
									ImmersiveIntelligence.logger.error("15 error at: "+h+" "+w+" "+l);
									return false;
								}
							}
						...
						else if(l >= 3&&l <= 5&&(w==-4||w==4))
						{
							if(l==4&&w==-4)
							{
								...
								{
									ImmersiveIntelligence.logger.error("16 error at: "+h+" "+w+" "+l);
									return false;
								}
							}
							else if(l==4&&w==4)
							{
								...
								{
									ImmersiveIntelligence.logger.error("17 error at: "+h+" "+w+" "+l);
									return false;
								}
							}
							else
							{
								if(!Utils.isBlockAt(world, pos, IEContent.blockStorage, BlockTypes_MetalsIE.STEEL.getMeta()))
								{
									ImmersiveIntelligence.logger.error("18 error at: "+h+" "+w+" "+l);
									return false;
								}
							}
						}
						else if(w==0&&l==0)
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockStoneDecoration, BlockTypes_StoneDecoration.CONCRETE.getMeta()))
							{
								ImmersiveIntelligence.logger.error("19 error at: "+h+" "+w+" "+l);
								return false;
							}
						}
						else
					...
					else if(h==1)
					{
						...
						else if(w >= -2&&w <= 2&&l >= 2&&l <= 6)
						{
							if(!Utils.isBlockAt(world, pos, IEContent.blockSheetmetal, BlockTypes_MetalsAll.STEEL.getMeta()))
							{
								ImmersiveIntelligence.logger.error("s error at: "+h+" "+w+" "+l);
								return false;
							}
						}
					...
					else
					{
						...
						{
							ImmersiveIntelligence.logger.error("0 error at: "+h+" "+w+" "+l);
							ImmersiveIntelligence.logger.error(pos);
							ImmersiveIntelligence.logger.error(world.getBlockState(pos).getBlock().getLocalizedName());
