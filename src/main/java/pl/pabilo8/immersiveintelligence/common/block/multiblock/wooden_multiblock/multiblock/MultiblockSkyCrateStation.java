...
public class MultiblockSkyCrateStation implements IMultiblock
{
	static final IngredientStack[] materials = new IngredientStack[]{
			...
			new IngredientStack(new ItemStack(IIContent.blockClothDecoration, 1, ClothDecorations.COIL_ROPE.getMeta())),
		...
		for(int h = 0; h < 3; h++)
		{
			for(int l = 0; l < 3; l++)
			{
				for(int w = 0; w < 3; w++)
				{
					...
					{
						if(l==1)
							structure[h][l][w] = new ItemStack(IIContent.blockClothDecoration, 1, ClothDecorations.COIL_ROPE.getMeta());
						else
		...
		for(int h = -1; h < 2; h++)
			for(int l = -1; l < 2; l++)
				for(int w = -1; w < 2; w++)
				{
					...
					world.setBlockState(pos2, IIContent.blockWoodenMultiblock.getStateFromMeta(IIBlockTypes_WoodenMultiblock.SKYCRATE_STATION.getMeta()));
			...
			for(int l = -1; l < 2; l++)
				for(int w = -1; w < 2; w++)
				{
					...
					if(h==-1&&w==-1)
					{
						if(l==0)
						{
							if(!Utils.isBlockAt(world, pos, IIContent.blockClothDecoration, ClothDecorations.COIL_ROPE.getMeta()))
							{
								return false;
							}
						}
