...
public class MultiblockFlagpole implements IMultiblock
{
	static final IngredientStack[] materials = new IngredientStack[]{
			new IngredientStack(new ItemStack(IIContent.blockConcreteDecoration, 9, ConcreteDecorations.CONCRETE_BRICKS.getMeta())),
			...
			new IngredientStack(new ItemStack(IIContent.blockWoodenFortification, 1, WoodenFortifications.WOODEN_STEEL_CHAIN_FENCE.getMeta())),
		...
		for(int h = 0; h < 8; h++)
		{
			for(int l = 0; l < 3; l++)
			{
				for(int w = 0; w < 3; w++)
				{
					...
					if(h==0)
					{
						structure[h][l][w] = new ItemStack(IIContent.blockConcreteDecoration, 1, ConcreteDecorations.CONCRETE_BRICKS.getMeta());
					}
					else if(h < 3)
					{
						if(w==1&&l==1)
					...
					else
					{
						...
						{
							if(w==0&&l==2)
								structure[h][l][w] = new ItemStack(IIContent.blockWoodenFortification, 1, WoodenFortifications.WOODEN_STEEL_CHAIN_FENCE.getMeta());
							else if(l==0)
		...
		for(int h = -3; h < 5; h++)
			for(int l = 0; l < 3; l++)
				for(int w = -1; w < 2; w++)
				{
					...
					if(h==-3)
					{
						if(!Utils.isBlockAt(world, pos, IIContent.blockConcreteDecoration, ConcreteDecorations.CONCRETE_BRICKS.getMeta()))
						{
							return false;
						}
					}
					...
					else
					{
						...
						else if(h==0)
						{
							if(w==-1&&l==2)
							{
								if(!Utils.isBlockAt(world, pos, IIContent.blockWoodenFortification, WoodenFortifications.WOODEN_STEEL_CHAIN_FENCE.getMeta()))
								{
									return false;
								}
							}
