package pl.pabilo8.immersiveintelligence.common.block.metal.conveyors;
...
public class ConveyorRubberExtract extends ConveyorBasic
{
	...
	public List<BakedQuad> modifyQuads(List<BakedQuad> baseModel, @Nullable TileEntity tile, EnumFacing facing)
	{
		...
		final TextureAtlasSprite texture_casing = ClientUtils.getSprite(ConveyorRubberExtract.texture_casing);
	...
	public boolean playerInteraction(TileEntity tile, EntityPlayer player, EnumHand hand, ItemStack heldItem, float hitX, float hitY, float hitZ, EnumFacing side)
	{
		...
		{
			if(this.transferTickrate==1)
				this.transferTickrate = 2;
			else if(this.transferTickrate==2)
			...
			if(this.transferTickrate==4)
				...
				this.transferTickrate = 30;
			else if(this.transferTickrate==30)
				this.transferTickrate = 40;
			else if(this.transferTickrate==40)
				this.transferTickrate = 1;
