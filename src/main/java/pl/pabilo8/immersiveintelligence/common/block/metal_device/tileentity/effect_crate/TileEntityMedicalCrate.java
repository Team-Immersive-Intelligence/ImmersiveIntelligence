...
public class TileEntityMedicalCrate extends TileEntityEffectCrate implements ITileDrop
{
	...
	{
		super.update();
		boolean u = IIUtils.handleBucketTankInteraction(tanks, inventory, 0, 1, 0, false, HEALTH_POTION);
	...
	boolean affectEntity(Entity entity, boolean upgraded)
	{
		if(!upgraded||(repairCrateEnergyPerAction <= energyStorage))
		{
			...
			{
				if(!((EntityLivingBase)entity).isPotionActive(IIPotions.medicalTreatment))
				{
					if(shouldHeal&&tanks[0].getFluidAmount() >= mediCrateFluidDrain)
					{
						...
						((EntityLivingBase)entity).addPotionEffect(new PotionEffect(IIPotions.medicalTreatment, upgraded?200: 400, upgraded?1: 0, true, true));
					}
					if(shouldBoost&&tanks[1].getFluidAmount() >= mediCrateFluidDrain)
					{
						tanks[1].drain(mediCrateFluidDrain, true);
						((EntityLivingBase)entity).addPotionEffect(new PotionEffect(IIPotions.ironWill, upgraded?200: 400, upgraded?1: 0, true, true));
					}
	...
	public int getGuiID()
	{
		return IIGUI.MEDIC_CRATE.ordinal();
	}
