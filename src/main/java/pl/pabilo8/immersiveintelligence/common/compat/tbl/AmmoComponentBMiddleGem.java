...
public class AmmoComponentBMiddleGem extends AmmoComponent
/**
...
// default tbl value: max 3 of weakness lasting 2 s.
// Given we are stuffing it into an explosive shell or a bullet, and gem ores are rare,and we are shooting it without return, may as well amplify.
	...
	public AmmoComponentBMiddleGem()
	{
		super("aqua_middlegem", 1f, ComponentRole.SPECIAL, IIColor.fromPackedARGB(0x0d2a82));
	}
	...
	{
		Item aqua_middlegem = Item.REGISTRY.getObject(new ResourceLocation("thebetweenlands", "aqua_middle_gem"));

		return new IngredientStack(new ItemStack(aqua_middlegem, 1));
	}
	@Override
	...
	{
		SoundEvent sound = ForgeRegistries.SOUND_EVENTS.getValue(ResLoc.of("thebetweenlands:mist_staff_vanish"));
		...
		for(EntityLivingBase e : entities)
		{
			...
			Potion mining_fatigue = Potion.REGISTRY.getObject(ResLoc.of("minecraft:mining_fatigue"));
			e.addPotionEffect(new PotionEffect(weakness, 160, 4));
			e.addPotionEffect(new PotionEffect(mining_fatigue, 160, 4));
			e.attackEntityFrom(DamageSource.DROWN, 8);
		}
		Vec3d v = dir.scale(-1);
		...
		cloud.setParticle(EnumParticleTypes.WATER_SPLASH);
