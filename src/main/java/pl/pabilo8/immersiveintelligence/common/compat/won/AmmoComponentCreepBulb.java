...
public class AmmoComponentCreepBulb extends AmmoComponent
{
	...
	{
		Item creepedbulb = Item.REGISTRY.getObject(new ResourceLocation("wyrmsofnyrus", "creepedbulb"));
		return new IngredientStack(new ItemStack(creepedbulb, 1));
		//creepbulbblock - for creepedbulb in block form
	}
	...
	public void onEffect(World world, Vec3d pos, Vec3d dir, ComponentEffectShape shape, NBTTagCompound tag, float componentSize, float multiplier, Entity owner)
	{
		...
		for(EntityLivingBase e : entities)
		{
			...
			e.addPotionEffect(new PotionEffect(weakness, 160, 1));
			e.addPotionEffect(new PotionEffect(IIPotions.radiation, 160, 2));
			e.attackEntityFrom(IIDamageSources.RADIATION_DAMAGE, 3);

			e.hurtResistantTime = 0;
		}

		final int endRad = (int)(4);
		final int biomeWasteland = Biome.getIdForBiome(IIContent.biomeWasteland);

		int wastelandRadius = (int)(4)*1; //somewhat neglectable contamination
