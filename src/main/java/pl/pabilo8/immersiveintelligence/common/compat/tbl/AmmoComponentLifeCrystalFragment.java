...
public class AmmoComponentLifeCrystalFragment extends AmmoComponent
{
	public AmmoComponentLifeCrystalFragment()
	{
		super("life_crystal_fragment", 0.42f, ComponentRole.SPECIAL, IIColor.fromPackedRGB(0xe2e2e2));
	}

	@Override
	public IngredientStack getMaterial()
	{
		return new IngredientStack("life_crystal_fragment");
	}
	...
	public void onEffect(World world, Vec3d pos, Vec3d dir, ComponentEffectShape shape, NBTTagCompound tag, float componentSize, float multiplier, Entity owner)
	{
		...
		new IIExplosion(world, owner, pos, dir,
				5, 25*multiplier, ComponentEffectShape.ORB, false, true, false)
				.doExplosion();
		...
		for(EntityLivingBase e1 : entities)
		{
			...
			e1.attackEntityFrom(DamageSource.MAGIC, 250);
			e1.addPotionEffect(new PotionEffect(IIPotions.radiation, 200, 5));
		}

		EntityLivingBase[] entities2 = world.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(ppos).grow(7)).toArray(new EntityLivingBase[0]);
		for(EntityLivingBase e1 : entities2)
		{
			...
			e1.attackEntityFrom(DamageSource.MAGIC, 5);
		}

		float radius = multiplier*5;
