	...
	public void onUpdate(ItemStack stack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected)
	{
		...
		if(isSelected)
		{
			...
			if(shouldReload)
			{
				...
				if(reloading==1)
					worldIn.playSound(null, entityIn.posX, entityIn.posY, entityIn.posZ, magazine.isEmpty()?IISounds.submachinegunReload: IISounds.submachinegunUnload, SoundCategory.PLAYERS, 0.35f, 1f);
	...
	public void onUsingTick(@Nonnull ItemStack stack, EntityLivingBase player, int count)
	{
		...
		if(ItemNBTHelper.getInt(stack, "fireDelay")==0)
		{
			...
			if(!worldIn.isRemote)
			{
				...
				Vec3d vec = IIUtils.getVectorForRotation(player.rotationPitch-recoilH, player.getRotationYawHead()+recoilV);
				...
				worldIn.playSound(null, player.posX+vec.x, player.posY+vec.y, player.posZ+vec.z, IISounds.submachinegunShot, SoundCategory.PLAYERS, 1.5f*noise, supressor?0.75f: 0f);

				IIPacketHandler.INSTANCE.sendToAllAround(
						new MessageParticleGunfire(player, supressor?0.5f: 1.5f),
						IIUtils.targetPointFromEntity(player, 32));
				...
				ItemStack cc = ((IAmmo)s2.getItem()).getCasingStack(1);
