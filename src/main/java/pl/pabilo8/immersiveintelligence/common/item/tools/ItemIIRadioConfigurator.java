...
public class ItemIIRadioConfigurator extends ItemIIBase implements IItemScrollable
{
	...
	{
		list.add(I18n.format(IILib.DESCRIPTION_KEY+"radio_configurator_"+(isBasic(stack)?"basic": "advanced")));
		list.add(I18n.format(IILib.DESCRIPTION_KEY+"radio_configurator_max_frequency", isBasic(stack)?IIConfig.radioBasicMaxFrequency: IIConfig.radioAdvancedMaxFrequency));
		list.add(I18n.format(IILib.DESCRIPTION_KEY+"radio_configurator_frequency", ItemNBTHelper.getInt(stack, "Frequency")));
	}
	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
	{
		if(!worldIn.isRemote&&worldIn.getTileEntity(pos) instanceof IRadioDevice)
		{
			...
			if(!player.isSneaking())
			{
				...
				{
					ImmersiveEngineering.packetHandler.sendTo(new MessageNoSpamChatComponents(new TextComponentTranslation(IILib.INFO_KEY+"frequency_invalid", IIConfig.radioBasicMaxFrequency)), (EntityPlayerMP)player);
				}
			}
			else
			{
	...
	public void onScroll(ItemStack stack, boolean forward, EntityPlayerMP player)
	{
		...
		SPacketTitle packet = new SPacketTitle(Type.ACTIONBAR, new TextComponentTranslation(IILib.DESCRIPTION_KEY+"radio_configurator_frequency", ItemNBTHelper.getInt(stack, "Frequency")), 0, 20, 0);
