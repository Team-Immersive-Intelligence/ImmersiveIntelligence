...
The project should automatically be recognized as a Gradle project and load the scripts, if it didn't load them, do it manually by using
`Gradle>Reload All Gradle Projects`.

Navigate to
`File>Project Structure` and set SDK to a Java 8 (1.8) SDK. In Team II we use Amazon Corretto 8 both locally and on the GitHub Build Action.
Navigate to `File>Settings>Build, Execution, Deployment>Build Tools>Gradle` and select 'Build and Run using' to
`IntelliJ`, set Gradle's Java version to the same Amazon Coretto 8.
...
Depending on your system, some additional settings may not have been applied. If you encounter issues, run
`gradlew additionalIntellijSettings`.
...
First, make sure java is installed and assigned to PATH, then in the root directory run the command:
...
To build a jarfile, use
`gradlew build`. Note that by default it won't be signed and will be flagged as an unofficial build in the game.  
...
If you wish to use these libraries in your project, message [@Pabilo8](https://github.com/Pabilo8/) directly at discord or
`pabilo (at) iiteam (dot) net`.